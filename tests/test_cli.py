...
class TestParsePlanArgs(unittest.TestCase):
    ...
    def test_parse_plan_with_schema_dir(self):
        ...
        with patch.object(
            sys, "argv", ["prog", "--env", "dev", "--schema-dir", "custom"]
        ):
            args = parse_plan_args("Test description")
            self.assertEqual(args.env, "dev")
    ...
    def test_parse_plan_migration_with_env_error(self):
        ...
        with patch.object(
            sys, "argv", ["prog", "--from", "dev", "--to", "staging", "--env", "prod"]
        ):
            with self.assertRaises(SystemExit):
...
class TestParseApplyArgs(unittest.TestCase):
    ...
    def test_parse_apply_with_schema_dir(self):
        """Test parsing with schema directory override."""
        with patch.object(
            sys, "argv", ["prog", "--env", "staging", "--schema-dir", "/custom/path"]
        ):
            args = parse_apply_args("Test description")
...
class TestSetupClient(unittest.TestCase):
    ...
    def test_setup_client_with_key_file(
        self, mock_from_args, mock_load_config, mock_gcloud
    ):
        """Test setup_client with key_file configuration."""
        # Setup mock workspace config
        env_config = EnvironmentConfig(
            name="dev", key_file="secrets/key.json", key_env=None, description=None
        )
        ...
        workspace_config.get_schema_dir.return_value = Path(
            "/test/firestore_schema/dev"
        )
        ...
        self.assertEqual(
            call_args["schema_dir"], str(Path("/test/firestore_schema/dev"))
        )
    ...
    @patch("firesync.cli.load_config")
    @patch("firesync.config.FiresyncConfig.from_args")
    def test_setup_client_with_key_env(
        self, mock_from_args, mock_load_config, mock_gcloud
    ):
        """Test setup_client with key_env configuration."""
        # Setup mock workspace config
        env_config = EnvironmentConfig(
            name="staging", key_file=None, key_env="GCP_KEY", description=None
        )
        workspace_config = MagicMock(spec=WorkspaceConfig)
        workspace_config.get_env.return_value = env_config
        workspace_config.config_dir = Path("/project")
        workspace_config.get_schema_dir.return_value = Path(
            "/project/firestore_schema/staging"
        )
        mock_load_config.return_value = workspace_config
        ...
        self.assertEqual(
            call_args["schema_dir"], str(Path("/project/firestore_schema/staging"))
        )

    @patch("firesync.cli.GCloudClient")
    @patch("firesync.cli.load_config")
    @patch("firesync.config.FiresyncConfig.from_args")
    def test_setup_client_with_schema_dir_override(
        self, mock_from_args, mock_load_config, mock_gcloud
    ):
        """Test setup_client with schema_dir override."""
        # Setup mock workspace config
        env_config = EnvironmentConfig(
            name="dev", key_file=None, key_env="GCP_KEY", description=None
        )
        workspace_config = MagicMock(spec=WorkspaceConfig)
        workspace_config.get_env.return_value = env_config
        workspace_config.config_dir = Path("/test")
        ...
        mock_from_args.assert_called_once_with(
            key_file=None, key_env="GCP_KEY", schema_dir="/custom/schemas"
        )
...
class TestNewCLIFeatures(unittest.TestCase):
    ...
    def test_parse_apply_migration_with_dry_run(self):
        ...
        with patch.object(
            sys, "argv", ["prog", "--from", "dev", "--to", "prod", "--dry-run"]
        ):
            args = parse_apply_args("Test description")
            self.assertEqual(args.from_env, "dev")
            self.assertEqual(args.to_env, "prod")
    ...
    @patch("firesync.cli.load_config")
    @patch("firesync.config.FiresyncConfig.from_args")
    def test_setup_client_with_dry_run(
        self, mock_from_args, mock_load_config, mock_gcloud
    ):
