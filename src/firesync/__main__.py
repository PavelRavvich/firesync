...
def create_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="firesync", description="Infrastructure as Code for Google Cloud Firestore"
    )

    parser.add_argument("--version", action="version", version="firesync 0.1.0")
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "env":
        ...
        env_args = [
            ...
            if arg not in ("--verbose", "-v", "--quiet", "-q")
        ]
        run_env(env_args)
        return
